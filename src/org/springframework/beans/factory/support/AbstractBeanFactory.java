...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport
	...
	/**
	 * Names of beans that have already been created at least once (using a
	 ...
	 *            non-null args value in any other case.
	 * @param typeCheckOnly
	 *            whether the instance is obtained for a type check, not for
	 *            actual use
	 * @return an instance of the bean
	 * @throws BeansException
	 *             if the bean could not be created
	...
	protected <T> T doGetBean(final String name, final Class<T> requiredType,
		...
		if (sharedInstance != null && args == null) {
			...
			/**
			 * 检查该Bean是否在当前BeanFactory中，否则从父BeanFactory中找以此找下去。
			 */
			...
			// on.
			// 获取当前Bean的所有依赖Bean，递归调用getBean()方法，直到一个没有任何依赖的Bean为止
			...
			// 创建prototype bean
				...
				try {
					...
					//生成Bean，createBean是抽象方法由子类AbstractAutowireCapableBeanFactory实现。
		...
		if (beanInstance != null) {
			...
			if (mbd.isSingleton()) {
				if (isFactoryBean(beanName, mbd)) {
					...
					FactoryBean<?> factoryBean = (FactoryBean<?>) getBean(FACTORY_BEAN_PREFIX
							+ beanName);
	...
	public boolean isPrototype(String name)
		...
		if (mbd.isPrototype()) {
			// In case of FactoryBean, return singleton status of created object
			// if not a dereference.
			return (!BeanFactoryUtils.isFactoryDereference(name) || isFactoryBean(
					beanName, mbd));
		} else {
			...
			if (isFactoryBean(beanName, mbd)) {
				final FactoryBean<?> factoryBean = (FactoryBean<?>) getBean(FACTORY_BEAN_PREFIX
						+ beanName);
		...
		if (beanInstance != null) {
			if (beanInstance instanceof FactoryBean) {
				if (!BeanFactoryUtils.isFactoryDereference(name)) {
					...
					return (type != null && ClassUtils.isAssignable(
							typeToMatch, type));
				} else {
					return ClassUtils.isAssignableValue(typeToMatch,
							beanInstance);
				}
			} else {
				return !BeanFactoryUtils.isFactoryDereference(name)
						&& ClassUtils.isAssignableValue(typeToMatch,
								beanInstance);
			}
		} else if (containsSingleton(beanName)
			...
			if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
				// No bean definition found in this factory -> delegate to
				// parent.
				return parentBeanFactory.isTypeMatch(originalBeanName(name),
						targetType);
			}
			...
			if (FactoryBean.class.isAssignableFrom(beanType)) {
				...
				// returned a non-FactoryBean
				// type but we nevertheless are being asked to dereference a
				// FactoryBean...
				// Let's check the original bean class and proceed with it if it
				// is a FactoryBean.
	...
	public String[] getAliases(String name) {
		...
		for (String retrievedAlias : retrievedAliases) {
			String alias = (factoryPrefix ? FACTORY_BEAN_PREFIX : "")
					+ retrievedAlias;
		...
		if (!containsSingleton(beanName) && !containsBeanDefinition(beanName)) {
			...
			if (parentBeanFactory != null) {
				aliases.addAll(Arrays.asList(parentBeanFactory
	...
	public void setParentBeanFactory(BeanFactory parentBeanFactory) {
		if (this.parentBeanFactory != null
				&& this.parentBeanFactory != parentBeanFactory) {
			throw new IllegalStateException(
					"Already associated with parent BeanFactory: "
	...
	public void registerScope(String scopeName, Scope scope) {
		...
		if (SCOPE_SINGLETON.equals(scopeName)
				|| SCOPE_PROTOTYPE.equals(scopeName)) {
	...
	public AccessControlContext getAccessControlContext() {
		return (this.securityContextProvider != null ? this.securityContextProvider
				.getAccessControlContext() : AccessController.getContext());
	}
	...
	public void copyConfigurationFrom(ConfigurableBeanFactory otherFactory) {
		...
		if (otherFactory instanceof AbstractBeanFactory) {
			...
			this.propertyEditorRegistrars
					.addAll(otherAbstractFactory.propertyEditorRegistrars);
			this.beanPostProcessors
					.addAll(otherAbstractFactory.beanPostProcessors);
	 ...
	 * <p>
	 * This {@code getMergedBeanDefinition} considers bean definition in
	 * ancestors as well.
	 * 
	 * @param name
	 ...
	 * <p>
	 * The default implementation register the prototype as currently in
	 * creation.
	 * 
	 * @param beanName
	 ...
	 * <p>
	 * The default implementation marks the prototype as not in creation
	 * anymore.
	 * 
	 * @param beanName
	...
	public void destroyBean(String beanName, Object beanInstance) {
		destroyBean(beanName, beanInstance,
				getMergedLocalBeanDefinition(beanName));
	}

	/**
	 * Destroy the given bean instance (usually a prototype instance obtained
	 * from this factory) according to the given bean definition.
	 * 
	 * @param beanName
	...
	protected void destroyBean(String beanName, Object beanInstance,
		...
		new DisposableBeanAdapter(beanInstance, beanName, mbd,
	...
	/**
	 * Initialize the given BeanWrapper with the custom editors registered with
	 * this factory. To be called for BeanWrappers that will create and populate
	 * bean instances.
	 * <p>
	...
	/**
	 * Initialize the given PropertyEditorRegistry with the custom editors that
	 * have been registered with this BeanFactory.
	 * <p>
	 * To be called for BeanWrappers that will create and populate bean
	 * instances, and for SimpleTypeConverter used for constructor argument and
	 * factory method type conversion.
	 * 
	 * @param registry
	 ...
	 *            the original bean definition (Root/ChildBeanDefinition)
	 * @param containingBd
	 *            the containing bean definition in case of inner bean, or
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			if (mbd == null) {
				if (bd.getParentName() == null) {
					...
					BeanDefinition pbd;
					try {
						String parentBeanName = transformedBeanName(bd
								.getParentName());
				...
				// A bean contained in a non-singleton bean cannot be a
				// singleton itself.
				// Let's correct this on the fly here, since this might be the
				// result of
				// parent-child merging for the outer bean, in which case the
				// original inner bean
				// definition will not have inherited the merged outer bean's
				// singleton status.
				...
				// Only cache the merged bean definition if we're already about
				// to create an
				// instance of the bean, or at least have already created an
				// instance before.
	...
	protected void checkMergedBeanDefinition(RootBeanDefinition mbd,
			String beanName, Object[] args) throws BeanDefinitionStoreException {
	...
	/**
	 * Remove the merged bean definition for the specified bean, recreating it
	 * on next access.
	 * 
	 * @param beanName
	...
	/**
	 * Resolve the bean class for the specified bean definition, resolving a
	 * bean class name into a Class reference (if necessary) and storing the
	 * resolved Class in the bean definition for further use.
	 * 
	 * @param mbd
	...
	protected Class<?> resolveBeanClass(final RootBeanDefinition mbd,
			String beanName, final Class<?>... typesToMatch)
			throws CannotLoadBeanClassException {
		try {
			if (mbd.hasBeanClass()) {
			...
			ClassNotFoundException ex = (ClassNotFoundException) pae
					.getException();
	...
	protected Object evaluateBeanDefinitionString(String value,
		...
		Scope scope = (beanDefinition != null ? getRegisteredScope(beanDefinition
				.getScope()) : null);
	 ...
	 * specified bean. Called by {@link #getType} and {@link #isTypeMatch}. Does
	 * not need to handle FactoryBeans specifically, since it is only supposed
	 * to operate on the raw bean type.
	 * <p>
	 * This implementation is simplistic in that it is not able to handle
	 * factory methods and InstantiationAwareBeanPostProcessors. It only
	 * predicts the bean type correctly for a standard bean. To be overridden in
	 * subclasses, applying more sophisticated type detection.
	 * 
	 * @param beanName
	 *            the name of the bean
	 * @param mbd
	...
	protected boolean isFactoryBean(String beanName, RootBeanDefinition mbd) {
		...
		return (beanType != null && FactoryBean.class
				.isAssignableFrom(beanType));
	}

	/**
	 * Determine the bean type for the given FactoryBean definition, as far as
	 * possible. Only called if there is no singleton instance registered for
	 * the target bean already.
	 * <p>
	 * The default implementation creates the FactoryBean via {@code getBean} to
	 * call its {@code getObjectType} method. Subclasses are encouraged to
	 * optimize this, typically by just instantiating the FactoryBean but not
	 * populating it yet, trying whether its {@code getObjectType} method
	 * already returns a type. If no type found, a full FactoryBean creation as
	...
	protected Class<?> getTypeForFactoryBean(String beanName,
		...
		try {
			FactoryBean<?> factoryBean = doGetBean(FACTORY_BEAN_PREFIX
					+ beanName, FactoryBean.class, null, true);
	...
	/**
	 * Remove the singleton instance (if any) for the given bean name, but only
	 * if it hasn't been used for other purposes than type checking.
	 * 
	 * @param beanName
	 *            the name of the bean
	...
	/**
	 * Get the object for the given bean instance, either the bean instance
	 * itself or its created object in case of a FactoryBean.
	 * 
	 * @param beanInstance
	...
	protected Object getObjectForBeanInstance(Object beanInstance, String name,
		...
		// Don't let calling code try to dereference the factory if the bean
		// isn't a factory.
		if (BeanFactoryUtils.isFactoryDereference(name)
		...
		// Now we have the bean instance, which may be a normal bean or a
		// FactoryBean.
		// If it's a FactoryBean, we use it to create a bean instance, unless
		// the
	...
	/**
	 * Determine whether the given bean name is already in use within this
	 * factory, i.e. whether there is a local bean or alias registered under
	 * this name or an inner bean created with this name.
	 * 
	 * @param beanName
	 *            the name to check
	 */
	 ...
	 * a specified destroy method and registered
	...
	protected boolean requiresDestruction(Object bean, RootBeanDefinition mbd) {
		return (bean != null && (DisposableBeanAdapter.hasDestroyMethod(bean,
				mbd) || hasDestructionAwareBeanPostProcessors()));
	}

	/**
	 * Add the given bean to the list of disposable beans in this factory,
	 * registering its DisposableBean interface and/or the given destroy method
	 * to be called on factory shutdown (if applicable). Only applies to
	 * singletons.
	 * 
	 * @param beanName
	 *            the name of the bean
	...
	protected void registerDisposableBeanIfNecessary(String beanName,
		...
		if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {
			if (mbd.isSingleton()) {
				// Register a DisposableBean implementation that performs all
				// destruction
	 ...
	 * name. Does not consider any hierarchy this factory may participate in.
	 * Invoked by {@code containsBean} when no cached singleton instance is
	 * found.
	 * <p>
	 * Depending on the nature of the concrete bean factory implementation, this
	 * operation might be expensive (for example, because of directory lookups
	 * in external registries). However, for listable bean factories, this
	 * usually just amounts to a local hash lookup: The operation is therefore
	 * part of the public interface there. The same implementation can serve for
	 * both this template method and the public interface method in that case.
	 * 
	 * @param beanName
	...
	/**
	 * Create a bean instance for the given bean definition. The bean definition
	 * will already have been merged with the parent definition in case of a
	 * child definition.
	 * <p>
	 * All the other methods in this class invoke this method, although beans
	 * may be cached after being instantiated by this method. All bean
