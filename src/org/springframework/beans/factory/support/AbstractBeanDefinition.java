...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	//是否单例Bean，默认是true，对BeanFactory不起作用，对applicationContext起作用，
	//applicationContext在启动过程中会 实例化Bean，这样可以提高getBean性能。
	private boolean singleton = true;
	
	//原型参数，表示每次getBean时都重新创建Bean，与singleton互斥。
	private boolean prototype = false;

	private boolean abstractFlag = false;
	//延迟加载，默认是false，对BeanFactory不起作用，对applicationContext起作用,
	//applicationContext在启动过程中会 实例化Bean，这样可以提高getBean性能。
